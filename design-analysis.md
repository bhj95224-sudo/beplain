...
- 디자인 원본: [https://www.figma.com/design/czX98xxkHXsWcyFj4krR8S/%EB%B0%95%ED%9A%A8%EC%A0%95?node-id=2349-1071&m=dev] (node "wrap", 1920x13439)
...
- 제목 폰트: **[KIM]Wildgag**(Figma 원본, 라이선스 파일 없음) → **Jua**(Google Fonts)로 대체. 시각적으로 가장 가까운 무료 폰트를 선택했으며 정확히 동일하지 않음
...
- 360px: 1열 스택, 장식 이미지(별/구슬/리본/뱃지) 대부분 숨김, 캐러셀은 가로 스크롤 유지
- 768px: 장점 카드 3열, 장식 이미지 일부 노출
- 1280px: 데스크톱 포스터형 레이아웃에 가깝게 장식 요소 확장 노출
- 360/768/1280 모두 Playwright로 `scrollWidth === clientWidth` 확인(가로 스크롤 없음)

## 인터랙션

- 소개 이동 바: `#story`로 앵커 이동 (스크롤 애니메이션은 `scroll-behavior:smooth`)
- 캐러셀: 이전/다음 버튼 클릭 시 `carousel_track.scrollBy` (JS로 확인, 262px 이동 검증됨)
...
- 말풍선: assets/bubble_green1.png, bubble_green2.png, bubble_blue.png, bubble_litegreen.png, bubblepink1.png, bubblepink2.png
...
- Playwright(headless Chromium, npx로 신규 설치)로 데스크톱(1280)/태블릿(768)/모바일(360) 스크린샷 및 인터랙션(캐러셀, 맨 위로 이동) 확인, 콘솔 에러 없음
